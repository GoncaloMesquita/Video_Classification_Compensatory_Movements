...
class MomentDino(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        for frame in x2:
            ...
            self.attention_weights.append(dino_output.attentions)
        
        dino_embedding = torch.stack(frame_embeddings, dim=0)
        dino_embedding = dino_embedding.permute(1, 0, 2)
        dino_projection = self.dino_projection(dino_embedding)
        
        combined_embedding = torch.cat((moment_proj, dino_projection), dim=-1)
