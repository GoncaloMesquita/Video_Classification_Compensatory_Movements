...
def training(model, dataloader, optimizer, criterion, device, save_dir, model_name, thresholds, clip_value, optuna, epoch, scaler):
    ...
    if epoch == 5 and model_name in ['moment', 'AcT', 'SkateFormer', 'moment+dino']:
        
        if model_name == 'moment+dino':
            for name, param in model.named_parameters():
                if any(substring in name for substring in ["dinov2.encoder.layer.11", "moment_model.encoder.block.7"]):
    ...
    with context_manager:
        ...
        for batch_idx, (inputs, targets, lengths, inputs2) in enumerate(val_loader):
            ...
            if saliency_map and mode == 'testing':
                ...
                attributions = ig.attribute(input, baselines=input * 0, target=None)
                # if model_name == 'LSTM':
                ...
                pseudo_labels.append(gradient_integrated( targets, lengths, criterion, model_name, model, batch_idx, patient, treshold_labels, ig))
