...
def training(model, dataloader, optimizer, criterion, device, save_dir, model_name, thresholds, clip_value, optuna, epoch, scaler):
    ...
    for batch_idx, (inputs, lengths, targets, inputs2) in enumerate(dataloader):
...
def validate(model, val_loader, criterion, device, mode, save_dir, model_name, thresholds, optuna, saliency_map, scaler, patient):
    ...
    if patient not in pseudo_labels:
        pseudo_labels[patient] = []
    ...
    with context_manager:
        ...
        for batch_idx, (inputs, lengths, targets, inputs2) in enumerate(val_loader):
            ...
            if saliency_map and mode == 'testing':
                ...
                # if model_name == 'LSTM':
                pseudo_labels[patient].append(gradient_normal(inputs, targets, outputs, save_dir, lengths, criterion, model_name, model, batch_idx, patient))
    ...
    if saliency_map and mode == 'testing':
        pseudo_labels_dir = f'dataset/pseudo_labels/vanilla_gradients_0.47/Patient_{patient}'
        with open(f"{pseudo_labels_dir}.pkl", 'wb') as f:
            pickle.dump(pseudo_labels[patient], f)
