...
def validate(model, val_loader, criterion, device, mode, save_dir, model_name, thresholds, optuna, saliency_map, scaler, patient):
    ...
    with context_manager:
        ...
        for batch_idx, (inputs, lengths, targets, inputs2) in enumerate(val_loader):
            ...
            if saliency_map and mode == 'testing' and batch_idx in [0, 12, 14 , 25, 31, 47, 51, 61]:
                
                # if model_name == 'LSTM':
                pseudo_labels[patient] = gradient_normal(inputs, targets, outputs, save_dir, lengths, criterion, model_name, model, batch_idx, patient)
