...
def objective(trial):
    ...
    clip_value = trial.suggest_float("clip_value", 0.1, 1.0, step=0.1)
    eta = trial.suggest_float("eta", 0.0001, 0.001)
    step_size = trial.suggest_int("step_size", 15, 15, step=5)
    threshold = [trial.suggest_float(f"threshold_label_{i+1}", 0.1, 0.5) for i in range(6)]
...
def train(args):
    ...
    for i in range(1):
        ...
        # model = create_model(args.model_name, args.input_size, args.hidden_size, args.num_layers, args.num_labels, args.dropout, f"{args.save_dir}/{args.model_name}_{i}_best.pth", "test", args.pretrained)
        # model.to(device)
...
if __name__ == "__main__":
    ...
    args.save_dir = f"{args.save_dir}/{args.model_name}_bs{args.batch_size}_lr{args.learning_rate}_hs{args.hidden_size}_gm{args.gamma}_th{'_'.join(map(str, args.threshold))}_ss{args.step_size}/"
    os.makedirs(args.save_dir, exist_ok=True)
