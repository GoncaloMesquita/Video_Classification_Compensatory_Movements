...
from visualization import gradient_normal, grad_cam, grad_cam_II, attention_weights, gradient_integrated
...
def training(model, dataloader, optimizer, criterion, device, save_dir, model_name, thresholds, clip_value, optuna, epoch, scaler):
    ...
    if epoch == 3 and model_name in ['moment', 'AcT', 'SkateFormer', 'moment+dino']:
    ...
    with context_manager:
        ...
        for batch_idx, (inputs, targets, lengths, inputs2) in enumerate(val_loader):
            ...
            if saliency_map and mode == 'testing':
                ...
                if method == 'grad_cam':
                    # target_layer = model.moment_model.encoder.block[7].layer[0].SelfAttention.o
                    target_layer = model.transformer.layers[-1].mha
                    ...
                    for label_idx in range(num_labels):
                        ...
                        attributions.append(grad_cam.attribute(inputs, target=label_idx))
                    grad_cam_II(inputs, targets, outputs, save_dir, lengths, criterion, model_name, model, batch_idx, patient, attributions)
