...
from models.base import TimeseriesOutputs
from models.embed import PatchEmbedding, Patching
        ...
        **kwargs,
        ...
        def forward_hook(module, input, output):
            if self.activations is None:
                self.activations = []
            self.activations.append(output[2])  # Capture forward activations

        def backward_hook(module, grad_input, grad_output):
            if self.gradients is None:
                self.gradients = []
            self.gradients.append(grad_output[0].detach())  # Capture gradients during backpropagation

        for name, module in self.encoder.named_modules():
            if "layer" in name and "SelfAttention" in name:  # Target self-attention layers
