...
def visualization(model, test_loader, device, model_name, patient, method, trials, n_batch, dataset_name):

    if method == 'ig':
        gradient_integrated(test_loader, model_name, model, patient, device, trials, n_batch, dataset_name)
    
    elif method == 'vg':
        vanilla_gradients(test_loader, model_name, model, patient, device, trials, n_batch, dataset_name)
...
def gradient_integrated(test_loader, model_name, model, patient, device, trials, n_batch, dataset_name):
...
def vanilla_gradients(test_loader, model_name, model, patient, device, trials, n_batch, dataset_name):
    ...
    if dataset_name == 'SERE':
        ...
        label_names = ['standing', 'crouching', 'walking',
                       ...
                       'pocket_in', 'sitting', 'using_phone_desk', 
                       'talking_on_phone_desk', 'standing_up', 
                       'carrying_light', 'carrying_heavy', 'Carrying_light']    atual_batch = [math.floor(trial / n_batch) for trial in trials]
