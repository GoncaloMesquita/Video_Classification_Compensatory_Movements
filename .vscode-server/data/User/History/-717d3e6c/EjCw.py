...
def visaulization(model, test_loader, device, model_name, patient, method, trials, n_batch):

    if method == 'ig':
        gradient_integrated(test_loader, model_name, model, patient, device, trials, n_batch)
    
    elif method == 'vg':
        vanilla_gradients(test_loader, model_name, model, patient, device, trials, n_batch)
            ...
            if atual_trial[trial_idx] in range(len(inputs)):
                ...
                for i in range(len(label_names)):
                    if model_name == 'moment':
                        ...
                        vg = Saliency(model)
                        vg_gradients = vg.attribute(inputs, baselines=inputs * 0, target=i, additional_forward_args=(lengths,))
