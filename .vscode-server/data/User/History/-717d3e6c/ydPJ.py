...
def gradient_integrated(test_loader, model_name, model, patient, device, treshold_labels, trials, n_batch):
    
    label_names = ['General C.', 'Shoulder C.', 'Shoulder Elevation', 'Exaggerated Shoulder Abd.', 'Trunk C.', 'Head C.']
    atual_batch, atual_trial = [math.floor(trial / n_batch) for trial in trials], [torch.abs(trial-n_batch) for trial in trials]
            ...
            if model_name == 'moment':
                ...
                for i in range(0, len(label_names)):
                    ...
                    if model_name != 'LSTM':
                        ...
                        sns.heatmap(map[0][0:lengths[0]].T.detach().cpu().numpy(), cmap='RdYlBu', ax=axes[label], cbar_kws={'label': 'Normalized Saliency'})
