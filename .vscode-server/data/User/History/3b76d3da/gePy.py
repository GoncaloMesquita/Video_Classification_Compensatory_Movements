...
def train_I( args):
    ...
    for i , (X_t, y_t, X_test, y_test) in enumerate (cross_val_data):
        ...
        # if i in [1, 6, 16] and args.mode == 'test' and not args.saliency_map:
        #     continue
        
                
        if i < 17 and args.mode == 'test' and not args.saliency_map:
            continue
        ...
        if args.mode == "train":
            ...
            for epoch in range(args.epochs):
                ...
                val_loss, targets , predictions, sigmoid_output = validate(model, val_loader, criterion, device, "validation", args.save_dir, args.model_name, args.threshold, args.optuna, args.saliency_map, scaler, i, args.treshold_labels)
        ...
        if i == 17 and args.mode == 'test' and args.saliency_map:
            args.checkpoint = 'Results/AcT/AcT_bs16_lr2.900e-06_hs1_th0.200_0.400_0.150_0.350_0.150_0.350_eta8.270e-04_ptTrue/saved_models/AcT_10_best.pth' 
        ...
        if args.saliency_map:
            pseudo_labels[m].append(validate(model, test_loader, criterion, device, "testing", args.save_dir, args.model_name, args.threshold, False, args.saliency_map, scaler, i, args.treshold_labels))

        else:   
            _ , targets , predictions, sigmoid_output = validate(model, test_loader, criterion, device, "testing", args.save_dir, args.model_name, args.threshold, False, args.saliency_map, scaler, i, args.treshold_labels)
...
def train_II(args):
    ...
    fold_save_dir = os.path.join(args.save_dir, f"{args.data_pseudo_dir}")
    os.makedirs(fold_save_dir, exist_ok=True)
