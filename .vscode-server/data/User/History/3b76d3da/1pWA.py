...
def objective(trial):
    ...
    data = load_data(args.data_label, args.data_skeletons, args.model_name, args.save_dir, args.input_size)
    ...
    for i , (X_t, y_t, X_test, y_test) in enumerate (data):
        ...
        train_loader = create_dataloader(X_train_standardized, y_t, batch_size, True, args.model_name, args.seq_size)
        val_loader = create_dataloader(X_val_standardized, y_val, batch_size, False, args.model_name, args.seq_size)
        
        model = create_model(args.model_name, args.input_size, hidden_size, num_layers, args.num_labels, dropout, args.checkpoint, args.mode, args.pretrained, device, args.seq_size)
        model.to(device)
...
def train_I(args, dataset_name):
    ...
    cross_val_data, index_keep, true_data = load_data(args.data_label, args.data_skeletons, args.num_patients, args.input_size, args.mode)    
    ...
    for i , (X_t, y_t, X_test, y_test) in enumerate (cross_val_data):
        ...
        if args.mode == "train" or mode == "train":
            ...
            train_loader = create_dataloader(X_t, y_t, args.batch_size, True, args.model_name,args.seq_size, args.trainII, X_t_2)
            val_loader = create_dataloader(X_val, y_val, args.batch_size, False, args.model_name,args.seq_size, args.trainII, X_val_2)
            ...
            model, criterion, optimizer, scheduler, scaler = create_model(args.model_name, args.input_size, args.hidden_size, args.num_layers, args.num_labels, args.dropout, args.checkpoint, args.mode, args.pretrained, device, args.learning_rate, args.eta, args.epochs, args.seq_size )
            model.to(device)
            ...
            if isinstance(model, nn.DataParallel):
                print("Model is using DataParallel")
            epoch_train_losses = []
            epoch_val_losses = []
            ...
            for epoch in tqdm(range(args.epochs), desc="Epochs"):
                ...
                plot_auc_curves(targets, sigmoid_output, i, n_splits, mean_fpr, "train", args.first_label, dataset_name)
                ...
                plot_auc_curves(targets, sigmoid_output, i, n_splits, mean_fpr, "validation", args.first_label, dataset_name)
            ...
            test_loader = create_dataloader(X_test, y_test, args.batch_size, False, args.model_name, args.seq_size, args.trainII, X_test_2, args.num_dimensions)
            model , criterion, optimizer, scheduler, scaler = create_model(args.model_name, args.input_size, args.hidden_size, args.num_layers, args.num_labels, args.dropout, args.checkpoint, "test", args.pretrained, device, args.learning_rate, args.eta, args.epochs, args.seq_size)
            model.to(device)
            # if torch.cuda.device_count() > 1:
            ...
            pseudo_label(model, test_loader, device, args.save_dir, args.model_name, i, args.treshold_labels, args.method, dataset_name, true_data, args.input_size)
            ...
            if i in args.vis_patients:
                test_loader = create_dataloader(X_test, y_test, args.batch_size, False, args.model_name, args.seq_size, args.trainII, X_test_2)
                model, criterion, optimizer, scheduler, scaler = create_model(args.model_name, args.input_size, args.hidden_size, args.num_layers, args.num_labels, args.dropout, args.checkpoint, "test", args.pretrained, device, args.learning_rate, args.eta, args.epochs, args.seq_size )
                model.to(device)
                # if torch.cuda.device_count() > 1:
                #     model = nn.DataParallel(model, device_ids=[1,2,3])
        
                visualization(model, test_loader, device, args.model_name, i, args.method ,args.vis_trials, args.batch_size, dataset_name, args.input_size)
        ...
        if args.mode == 'test' or mode == 'test':    
            ...
            auc_cross = plot_auc_curves(targets, sigmoid_output, i, n_splits, mean_fpr, "testing", args.first_label, dataset_name)
    ...
    plot_auc_test(auc_test, args.save_dir, args.model_name, mean_fpr, args.first_label, dataset_name)
        

def train_II(args, dataset_name):
    ...
    args.save_dir = os.path.join(args.save_dir, f"{pseudo_dir_parts[-4]}/{pseudo_dir_parts[-3]}/{pseudo_dir_parts[-2]}")
    fold_save_dir = os.path.join(args.save_dir, f"saved_models")
    os.makedirs(fold_save_dir, exist_ok=True)
    
    device = torch.device(f'cuda:{args.n_device}' if torch.cuda.is_available() else 'cpu')
    
    cross_val_data = load_pseudo_label(args.data_skeletons, args.data_true_dir, args.data_pseudo_dir, args.first_label, args.true_labels, args.model_name, args.num_patients, args.input_size)    
    ...
    plot_auc_test(auc_test, args.save_dir, args.model_name, mean_fpr, args.trainII, dataset_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seq_size", type=int, default=1876, help="Sequence size")
    parser.add_argument("--num_dimensions", type=int, default=1, help="Number of frames")
    ...
    parser.add_argument("--vis_trials", type=int, nargs='+', default=[0,0,0], help="Trials to visualize")
    ...
    dataset_name = second_folder.split('_')[0]
    args.save_dir = f"{args.save_dir}/{dataset_name}/{args.model_name}"
    ...
    if args.trainII:
        train_II(args, dataset_name)
    else:
        train_I(args, dataset_name)
