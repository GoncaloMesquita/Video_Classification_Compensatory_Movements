...
def gradient_integrated(targets, lenghts, criterion, model_name, model, trial, patient, treshold_labels, ig_gradients):
    ...
    for label in range(len(label_names)):
        ...
        slc = slc[:, 0:lenghts[0], :]
        if model_name != 'LSTM':
            slc = smooth_gradients(slc, kernel_size=5)
...
def grad_cam_II(input, targets, outputs, output_dir, lengths, criterion, model_name, model, trial, patient, attributions):
    ...
    for label in range(num_labels):
        ...
        attention_weights_mean = layer_gradients_attr.mean(dim=1)  # Shape: [batch_size, seq_len, seq_len]
        ...
        # Expand dimensions to match expected shape for plotting
        saliency = saliency.unsqueeze(0)  # Shape: [1, batch_size, seq_len]

        # Interpolate to match desired size (e.g., number of joints)
        desired_joint_count = 33  # Adjust based on your data
        saliency = torch.nn.functional.interpolate(
            saliency,
            size=(lengths[0], desired_joint_count),
            mode='nearest'
        ).squeeze(0)  # Shape: [batch_size, seq_len, desired_joint_count]
        ...
        if patient == 17 and trial in [1, 15, 25, 35, 45, 55, 65]:
            ...
            fig2.suptitle(f"{model_name}: Sum of Saliency for Patient {patient + 1}:  Trial {trial}", fontsize=16, weight='bold')
            fig2.tight_layout(rect=[0, 0, 1, 0.96])
            fig2.savefig(f"saliency_maps/grad_cam_II/{model_name}_patient_{patient}_trial_{trial}_sum_grad_cam_II.png", dpi=300)
        plt.close(fig2)
