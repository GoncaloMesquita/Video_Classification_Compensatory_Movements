...
def gradient_normal(inputs, targets, outputs, output_dir, lengths, criterion, model_name, model, trial, patient, threshold_labels):
    label_names = ['General C.', 'Shoulder C.', 'Shoulder Elevation', 'Exaggerated Shoulder Abd.', 'Trunk C.', 'Head C.']
    
    # Initialize figure for multi-label saliency maps
    fig, axes = plt.subplots(2, 3, figsize=(15, 10))
    axes = axes.flatten()
    
    active_labels_str = "No compensation"  # Default for inactive labels
    binary_map = None  # Binary map for thresholding
    ...
    for label_idx, label_name in enumerate(label_names):
        ...
        loss = criterion(outputs[:, label_idx], targets[:, label_idx])
        
        # Backpropagate to compute gradients
        loss.backward(retain_graph=True)  # Retain graph for multi-label setup
        ...
        if model_name == 'moment':
            ...
            saliency_map = torch.relu(gradients)
        
        # Truncate saliency map to valid input lengths
        saliency_map = saliency_map[:, :lengths[0], :]
        
        # Smooth gradients for non-LSTM models
        if model_name != 'LSTM':
            saliency_map = smooth_gradients(saliency_map, kernel_size=10)
        
        # Reshape and interpolate saliency for visualization
        saliency_map = torch.nn.functional.interpolate(saliency_map.unsqueeze(0), size=(saliency_map.shape[1], 33), mode='nearest').squeeze(0)
        
        # Normalize the saliency map (avoid division by zero)
        normalized_map = torch.zeros_like(saliency_map)
        for i in range(saliency_map.shape[0]):
            max_val = saliency_map[i].max()
            min_val = saliency_map[i].min()
            if max_val > min_val:  # Avoid division by zero
                normalized_map[i] = (saliency_map[i] - min_val) / (max_val - min_val)
        
        # Plot individual saliency maps for each label
        sns.heatmap(
            normalized_map[0][:lengths[0]].T.detach().cpu().numpy(),
            cmap='RdYlBu',
            ax=axes[label_idx],
            cbar_kws={'label': 'Normalized Saliency'}
        )
        axes[label_idx].set_xlabel("Frames", fontsize=12)
        axes[label_idx].set_ylabel("Joints", fontsize=12)
        axes[label_idx].set_title(f"Label: {label_name}", fontsize=14)
        axes[label_idx].tick_params(axis='both', which='major', labelsize=10)
        
        # Save additional visualization for label 0
        if label_idx == 0:
            fig2, ax2 = plt.subplots(1, 1, figsize=(10, 5))
            head_joints = normalized_map[0][:lengths[0], :].sum(dim=1).detach().cpu().numpy()
            ax2.plot(head_joints)
            ax2.set_title("Sum of Saliency Across Joints", fontsize=14)
            ax2.set_xlabel("Frames", fontsize=12)
            ax2.set_ylabel("Sum of Saliency", fontsize=12)
            plt.tight_layout()
            plt.savefig(f"{output_dir}/vanilla_gradients/{model_name}_patient_{patient}_trial_{trial}_label_{label_idx}_VG.png", dpi=300)
            plt.close(fig2)
        
        # Generate binary pseudo-label map based on thresholds
        if label_idx == 0:
            binary_map = (normalized_map[0][:lengths[0]].sum(dim=1) < threshold_labels).int().detach().cpu().numpy()
        
        # Collect active labels for the trial
        if targets[0, label_idx] > 0:
            active_labels_str = ", ".join([label_names[i] for i in range(len(label_names)) if targets[0, i] > 0])
    
    # Save the main saliency map figure for all labels
    plt.suptitle(
        f"{model_name}: Saliency Maps of Patient {patient + 1}:  Trial {trial} \n Active Labels: {active_labels_str}",
        fontsize=16, weight='bold'
    )
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(f"{output_dir}/vanilla_gradients/{model_name}_patient_{patient}_trial_{trial}_VG.png", dpi=300)
    plt.close(fig)
