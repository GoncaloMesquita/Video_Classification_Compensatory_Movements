...
def grad_cam_II(input, targets, outputs, output_dir, lengths, criterion, model_name, model, trial, patient):
    ...
    for label in range(len(label_names)):   
        ...
        activations_mean_2 = activations[2].mean(dim=1)  
        activations_mean_3 = activations[3].mean(dim=1)
        
        weights_attention = activations_mean_2[:,0] * activations_mean_3[:,0]
        ...
        plt.savefig(f'saliency_maps/grad_cam_II/grad-cam_activation_2_3.png', dpi=300)
        plt.show()
