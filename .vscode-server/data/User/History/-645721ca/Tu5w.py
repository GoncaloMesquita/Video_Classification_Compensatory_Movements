...
def rollout(model, model_name):
    ...
    if model_name == 'moment':
        layers = model.module.moment_model.attention_maps[0]
...
def grad_cam(input, targets, outputs, output_dir, lenghts, criterion, model_name, model, trial, patient):
    ...
    for label in range(len(label_names)):
        ...
        if model_name == 'moment':
            attention_weights_mean = torch.nn.functional.interpolate(attention_weights_mean.unsqueeze(0), size=(513), mode='nearest').squeeze(0) 
