...
def gradient_normal(input, targets, outputs, output_dir, lenghts, criterion, model_name, model, trial, patient, treshold_labels):\
    ...
    for label in range(len(label_names)):
        ...
        # if patient == 14 and trial in [1, 15, 25, 35, 45, 55, 65]:
...
def grad_cam_II(input, targets, outputs, output_dir, lengths, criterion, model_name, model, trial, patient):
    ...
    for label in range(len(label_names)):   
        ...
        weights_attention = cls_token * gradients[:,0]
        
        interpolated_map = F.interpolate(weights_attention.unsqueeze(0).unsqueeze(0),  size=(33, 512),  align_corners=False).squeeze(0).squeeze(0)  # Shape: [33, lengths]
        ...
        plt.savefig(f'saliency_maps/grad_cam_II/grad-cam_activation_2_gradient_cls_token_head_mean.png', dpi=300)
        plt.show()
