...
def training(model, dataloader, optimizer, criterion, device, save_dir, model_name, treshold, clip_value, optuna=False, pretrained=False):
    
    if pretrained:
        # Freeze all layers except the first and last
        for name, param in model.named_parameters():
            if not (name.startswith('layer1') or name.startswith('layer_last')):
