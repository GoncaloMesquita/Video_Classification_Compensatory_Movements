...
def training(model, dataloader, optimizer, criterion, device, save_dir, model_name, treshold):
    ...
    for batch_idx, (inputs, lengths, targets) in enumerate(dataloader):
        ...
        predicted = torch.sigmoid(outputs) > float(treshold)
...
def validate(model, val_loader, criterion, device, mode, save_dir, model_name, treshold):
