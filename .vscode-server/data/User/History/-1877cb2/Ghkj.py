...
def create_model(model_name, input_size, hidden_size, num_layers, num_labels, dropout, checkpoint, mode, pretrained, device, learning_rate, eta, epochs):
    ...
    if model_name == 'LSTM':
        ...
        if mode == 'train':
            ...
            for name, param in model.named_parameters():
                if name.startswith(('moment_model.head.', 'moment_model.encoder.block.7', 'moment_model.encoder.block.6', 'moment_model.encoder.block.5' ,'moment_model.encoder.block.4','moment_model.encoder.block.3','moment_model.encoder.block.2','moment_model.encoder.block.1' )):
