...
class SkateFormer(nn.Module):
    ...
    def forward(self, input, index_t):
        ...
        if self.index_t:
            self.joint_person_embedding = nn.Parameter(torch.zeros(self.embed_dim, self.num_points * self.num_people))
            trunc_normal_(self.joint_person_embedding, std=.02)
        else:
            self.joint_person_temporal_embedding = nn.Parameter(
                torch.zeros(1, self.embed_dim, self.num_frames, self.num_points * self.num_people))
