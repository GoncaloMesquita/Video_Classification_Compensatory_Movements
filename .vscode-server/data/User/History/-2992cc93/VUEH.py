...
class SkateFormerBlock(nn.Module):
    ...
    def forward(self, input, mask):
        ...
        for i in range(len(self.partition_function)):
            ...
            y.append(self.reverse_function[i](self.attention[i](input_partitioned, mask), (T, V), self.partition_size[i]))
...
class SkateFormerBlockDS(nn.Module):
    ...
    def forward(self, input, mask):
        if self.downsample is not None:
            output = self.transformer(self.downsample(input), mask)
        else:
            output = self.transformer(input, mask)
        return output
...
class SkateFormer(nn.Module):
    ...
    def forward_features(self, input, mask):
        output = input
        for stage in self.stages:
            output = stage(output, mask)
        return output
    ...
    def forward(self, input, index_t):
        input = input.reshape(input.shape[0], 3, input.shape[1], 33, 1)
        B, C, T, V, M = input.shape
        
        lengh = index_t
        mask = create_padding_mask(lengh, T)
        ...
        output = self.forward_features(output, mask)
