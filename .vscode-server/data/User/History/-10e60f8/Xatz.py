...
def normalize_skeleton(ske):
    """
    Normalize skeleton sequences by:
    1. Centering each frame at the midpoint between hip joints (11 and 12)
    2. Scaling based on the distance between shoulders (5 and 6)
    
    Args:
        ske: Dictionary of skeleton sequences, where each key is a person ID and each value is a list of videos.
             Each video is a list of frames, and each frame contains joint coordinates in format [x,y,confidence].
    
    Returns:
        Normalized skeleton sequences with the same structure
    """
    ...
    for person_id, videos in ske.items():
        ...
        for video in videos:
            ...
            first_frame = np.array(video[0]).reshape(-1, 3)
