...
def load_pseudo_label(data_dir_skeletons, data_true_dir, data_pseudo_dir, first_label, true_labels, model_name, num_patients, input_size, n_dim):
    ...
    if data_true_dir.rsplit('/', 1)[1][0:5]  == 'MMAct': 
        ske = normalize_skeleton(ske, n_dim)     
    ske = {key: moving_average_filter(value) for key, value in ske.items()}
    ske = {key: initial_distance(value, input_size, n_dim) for key, value in ske.items()}    
