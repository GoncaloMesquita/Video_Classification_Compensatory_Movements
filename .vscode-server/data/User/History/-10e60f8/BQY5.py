...
def initial_distance(skeletons, size):
    features = []
...
def normalize_skeleton(skeletons):
    """
    Normalize skeleton sequences by centering each frame around a central joint.
    
    Args:
        skeletons: List of skeleton sequences, where each sequence is a list of frames,
                  and each frame contains joint coordinates in shape [num_joints, 3]
    ...
    for sequence in skeletons:
        ...
        for frame in sequence:
            ...
            frame_arr = np.array(frame).reshape(-1, 3)
            
            # Define central joint (typically hip or spine center)
            # Common choices:
            # - For COCO format: typically joint index 0 (mid-hip)
            # - For MPII format: typically joint index 6 (pelvis)
            # - For NTU RGB+D: typically joint index 1 (middle of spine)
            # Adjust this index based on your skeleton format
            central_joint_idx = 0  # Adjust this based on your skeleton format
            central_joint = frame_arr[central_joint_idx].copy()
            
            # Subtract central joint coordinates from all joints
            normalized_frame = frame_arr - central_joint
            
            # Optional: Scale the skeleton to have a consistent size
            # Example: normalize by the distance between shoulders or hip width
            # reference_dist = np.linalg.norm(normalized_frame[5] - normalized_frame[6])  # shoulder distance
            # if reference_dist > 0:
            #     normalized_frame = normalized_frame / reference_dist
            
            # Reshape back to original format
            normalized_sequence.append(normalized_frame.reshape(frame_arr.shape).tolist())
            
        normalized_skeletons.append(normalized_sequence)
    
    return normalized_skeletons


def load_data(data_dir_labels, data_dir_skeletons, num_patients, input_size, mode):
    ...
    # Remove None data points from skeletons
