...
def gradient_normal(input, targets, outputs, output_dir, lenghts, criterion, model_name, model, trial, patient):
    ...
    for label in range(len(label_names)):
        ...
        if label == 0:
            ...
            axes2.set_title("Head Joints (0-10)")
            axes2.set_xlabel("Frames")
            axes2.set_ylabel("Sum of Saliency")
            
            plt.tight_layout()
            plt.savefig(f"saliency_maps/vanila_gradients/{model_name}_patient_{patient}_trial_{trial}_label_{label}_plots.png", dpi=300)
            plt.close(fig2)
            
            binary_map = (map[0].sum(dim=1) > 0.47).int().detach().cpu().numpy()
    ...
    plt.savefig(f"saliency_maps/{model_name}_patient_{patient}_trial_{trial}_normal_gradient.png", dpi=300)
    plt.close()
