...
def gradient_normal(input, targets, outputs, output_dir, lenghts, criterion, model_name, model, trial, patient, treshold_labels):
    ...
    for label in range(len(label_names)):
        ...
        active_labels = [label_names[i] for i in range(0,len(label_names)) if targets[:, i] > 0]
        ...
        if label == 0:
            ...
            plt.tight_layout()
            plt.savefig(f"saliency_maps/vanilla_gradients/{model_name}_patient_{patient}_trial_{trial}_label_{label}_plots.png", dpi=300)
            plt.close(fig2)
