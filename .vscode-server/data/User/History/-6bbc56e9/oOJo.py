...
def gradient_integrated(targets, lenghts, criterion, model_name, model, trial, patient, treshold_labels, ig_gradients):
    ...
    fig2, axes2 = plt.subplots(2, 3, figsize=(10, 15))
    ...
    for label in range(len(label_names)):
        ...
        if patient == 17 and trial in [1, 15, 25, 35, 45, 55, 65]:
            ...
            axes2[label].set_title("Joints")
            axes2[label].set_xlabel("Frames")
            axes2[label].set_ylabel("Sum of Saliency")
            
            plt.tight_layout()
            plt.savefig(f"saliency_maps/intagrated_gradients/{model_name}_patient_{patient}_trial_{trial}_label_{label}_plots_IG.png", dpi=300)
            plt.close(fig2)
    ...
    if patient == 17 and trial in [1, 15, 25, 35, 45, 55, 65]:
        ...
        plt.savefig(f"saliency_maps/intagrated_gradients/{model_name}_patient_{patient}_trial_{trial}_IG.png", dpi=300)
        plt.close()
