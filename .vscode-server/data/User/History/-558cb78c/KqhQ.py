...
def gradient_integrated(targets, lenghts, criterion, model_name, model, trial, patient, treshold_labels, ig_gradients):
    ...
    for label in range(len(label_names)):
        ...
        if patient in [3, 11, 17] :
            ...
            if trial in trial_1:
                ...
                for j in trial_batch[trial]:
                    ...
                    active_labels = [label_names[i] for i in range(1, len(label_names)) if targets[j, i] > 0]
                    if active_labels:
        ...
        if label == 0:
            ...
            binary_map = []
            for m in range(0, 32):
                binary_map.append((map[m][0:lengths[m]].sum(dim=1) < treshold_labels).int().detach().cpu().numpy())
                print(map[m][0:lengths[m]].shape)
