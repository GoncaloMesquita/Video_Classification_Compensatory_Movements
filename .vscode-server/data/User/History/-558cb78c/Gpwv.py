...
def gradient_normal(input, targets, outputs, output_dir, lengths, criterion, model_name, model, trial, patient, treshold_labels):
    ...
    for label in range(len(label_names)):
        ...
        # Compute gradients w.r.t. input without retaining the graph
        grads = torch.autograd.grad(loss, input, retain_graph=False)[0]
        ...
        # Interpolate saliency map to a fixed size
        saliency = torch.nn.functional.interpolate(
            slc.unsqueeze(0), size=(slc.shape[1], 33), mode='nearest'
        ).squeeze(0)
        
        # Normalize the saliency map
        map = torch.zeros_like(saliency)
        for i in range(saliency.shape[0]):
            map[i] = (saliency[i] - saliency[i].min()) / (saliency[i].max() - saliency[i].min() + 1e-8)
        
        # Visualization for specific patients and trials
        if patient == 0 and trial in [1, 15, 25, 35, 45, 55, 65]:
            active_labels = [label_names[i] for i in range(1, len(label_names)) if targets[:, i] > 0]
            active_labels_str = ", ".join(active_labels) if active_labels else "No compensation"
            
            sns.heatmap(
                map[0][:lengths[0]].T.detach().cpu().numpy(),
                cmap='RdYlBu',
                ax=axes[label],
            ...
            if label == 0:
                ...
                head_joints = map[0][:lengths[0], :].sum(dim=1).detach().cpu().numpy()
                axes2.plot(head_joints)
                axes2.set_title("Sum of Saliency over Joints")
                axes2.set_xlabel("Frames")
                axes2.set_ylabel("Sum of Saliency")
                plt.tight_layout()
                plt.savefig(
                    f"saliency_maps/vanilla_gradients/{model_name}_patient_{patient}_trial_{trial}_label_{label}_plots_VG.png",
                    dpi=300
                )
                plt.close(fig2)
        
        if label == 0:
            binary_map = (map[0][:lengths[0]].sum(dim=1) < treshold_labels).int().detach().cpu().numpy()
    ...
    if patient == 0 and trial in [1, 15, 25, 35, 45, 55, 65]:
        ...
        plt.savefig(
            f"saliency_maps/vanilla_gradients/{model_name}_patient_{patient}_trial_{trial}_VG.png",
            dpi=300
        )
        plt.close()
