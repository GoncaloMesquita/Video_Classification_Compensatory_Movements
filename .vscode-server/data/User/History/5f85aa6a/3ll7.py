...
def create_dataloader(x, y, batch_size, shuffle, model_name, trainII, x2):
    ...
    if trainII:
        ...
        dataset = CustomDataset(x, y, model_name, trainII, x2) 
        dataloader = DataLoader(dataset,batch_size=batch_size, collate_fn=collate_fn, shuffle=shuffle, num_workers=2, pin_memory=True)
...
def collate_fn(batch):
    ...
    if sequences2[0] is None:
        return padded_sequences, lengths, torch.tensor(labels), None
...
def plot_auc_curves(targets, predictions, ax, fold, n_splits, mean_fpr, mode, type_train):
...
def load_pseudo_label(data_dir_skeletons, data_true_dir, data_pseudo_dir, first_label):
    ...
    ske_list = torch.tensor([frame for i in range(1, 19) for trial in ske[i] for frame in trial])
     
    data_true_list = torch.tensor([frame for i in range(1, 19) for trial in data_true[i] for frame in trial])

    data_pseudo_list = torch.tensor([frame for i in range(1, 19) for trial in data_pseudo[i] for frame in trial])
    
    if first_label:
        data_true_list = data_true_list[:, 0]
        
        
    X_train_val, X_test, y_train_val, y_test = train_test_split(X, y_true, test_size=0.10, random_state=10)

    if args.true_labels:
        X_t, X_val, y_t, y_val = train_test_split(X_train_val, y_train_val, test_size=0.15, random_state=10)
    else:
        _, _, y_train_val_pseudo, _ = train_test_split(X, y_pseudo, test_size=0.10, random_state=10)
        X_t, X_val, y_t, y_val = train_test_split(X_train_val, y_train_val_pseudo, test_size=0.15, random_state=10)
        
    return ske_list, data_true_list.unsqueeze(-1), data_pseudo_list