...
def load_pseudo_label(data_dir_skeletons, data_true_dir, data_pseudo_dir, first_label, true_labels):
    ...
    for i in range(1, 19):
        ...
        # Train set (excluding test set)
        if true_labels:
            train_ske = [frame for j in range(1, 19) if j != i for trial in ske[j] for frame in trial]
            train_labels = [frame for j in range(1, 19) if j != i for trial in data_true[j] for frame in trial]
        else:
            train_ske = [frame for j in range(1, 19) if j != i for trial in ske[j] for frame in trial]
            train_labels = [data_pseudo[j] for j in range(1, 19) if j != i]
