...
def pseudo_label(model, test_loader, criterion, device, mode, save_dir, model_name, thresholds, saliency_map, scaler, patient, treshold_labels, method):

    pseudo_labels = []
    
    if saliency_map and mode == 'testing':
        if method == 'ig':
            pseudo_labels.append(gradient_integrated(test_loader, criterion, model_name, model, patient, device))
        
        elif method == 'vg':
            pseudo_labels.append(gradient_normal(test_loader, criterion, model_name, model, patient, device))
        
        elif method == 'grad_cam':
            pseudo_labels.append(grad_cam(test_loader, criterion, model_name, model, patient, device))
    ...
    return pseudo_labels
            ...
            #             # attributions.append(ig.attribute(inputs, baselines=inputs * 0, target=0, additional_forward_args=lengths  ))

            #         attributions.append(integrated_gradients(inputs, model, lengths, 0, baseline=None, steps=5))
...
def gradient_integrated(test_loader, criterion, model_name, model, patient, device):
    ...
    for batch_idx, (inputs, targets, lengths, inputs2) in enumerate(test_loader):
        ...
        if model_name == 'moment':
            ...
            ig.attribute(inputs, baselines=inputs * 0, target=0, additional_forward_args=lengths
