    ...
    for batch_idx, (inputs, targets, lengths, inputs2) in enumerate(test_loader):
        ...
        if len(treshold_labels) == 1:
            ...
            for i in range(map.shape[0]):
                ...
                values = map[i][:lengths[i]].sum(axis=1).detach().cpu().numpy()  # Use axis=1 for sum if you have multi-dimensional data
                bi_map = np.full(values.shape, np.nan)  # Default to NaN
                
                # Apply the conditions to set binary values
                bi_map[values > treshold_labels[1]] = 0  # Values greater than treshold1 are set to 0
                bi_map[values < treshold_labels[0]] = 1  # Values less than treshold2 are set to 1
                
                # Store the result for this row in binary_map
