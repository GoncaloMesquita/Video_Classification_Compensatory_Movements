...
def save_pseudo_labels(pseudo_labels, save_dir, model_name, patient, method, threshold_labels):
    
    label = [label for sublist in pseudo_labels for label in sublist]
    if len(treshold_labels) == 1:
        method_type = 'method1'
        thr = threshold_labels[0]
    else:
        method_type = 'method2'
        thr = '{threshold_labels[0]}' + '_' + f'{threshold_labels[1]}'
    
    path = f'dataset/pseudo_labels/{model_name}/{method_type}/{method}_{threshold_labels}'
    os.makedirs(path, exist_ok=True)
    for m, labels in enumerate(label):
