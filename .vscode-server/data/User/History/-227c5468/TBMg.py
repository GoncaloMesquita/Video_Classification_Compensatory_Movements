...
def pseudo_label(model, test_loader, device, save_dir, model_name, patient, treshold_labels, method, dataset_name, true_data, input_size):
    ...
    if method == 'ig':
        pseudo_labels.append(gradient_integrated(test_loader, model_name, model, patient, device, treshold_labels, input_size, dataset_name))
    
    elif method == 'vg':
        pseudo_labels.append(vanilla_gradients(test_loader, model_name, model, patient, device, treshold_labels, input_size, dataset_name))
...
def vanilla_gradients(test_loader,  model_name, model, patient, device, treshold_labels, input_size, dataset_name):
    ...
    for batch_idx, (inputs, targets, lengths, inputs2) in enumerate(test_loader):
        ...
        map_labels = map_labels.view(map_labels.shape[1],map_labels.shape[2],map_labels.shape[3],map_labels.shape[0] )
        # 1) Threshold per label
        ...
        # 2) Average those thresholds
    thresholds_t = torch.stack(thresholds)      # shape: [num_labels]
    avg_threshold = thresholds_t.mean(axis=0)         # single scalar
...
#             map_labels = torch.stack(map_labels)  # stack all maps for each label
#             # map_labels = map_labels.reshape(map_labels.shape[1],map_labels.shape[2],map_labels.shape[3],map_labels.shape[0] )
#             # 1) Threshold per label
#             for j in range(map_labels.shape[0]):
