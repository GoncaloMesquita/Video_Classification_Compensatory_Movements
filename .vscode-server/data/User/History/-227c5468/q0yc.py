...
def pseudo_label(model, test_loader, criterion, device, mode, save_dir, model_name, thresholds, saliency_map, scaler, patient, treshold_labels, method):
    ...
    if saliency_map and mode == 'testing':
        if method == 'ig':
            pseudo_labels.append(gradient_integrated(test_loader criterion, model_name, model, patient, treshold_labels, attributions))
        
        elif method == 'vg':
            pseudo_labels.append(gradient_normal(test_loader,criterion, model_name, model, patient, treshold_labels))
        
        elif method == 'grad_cam':
            pseudo_labels.(grad_cam(test_loader outputs, save_dir, lengths, criterion, model_name, model, batch_idx, patient))
