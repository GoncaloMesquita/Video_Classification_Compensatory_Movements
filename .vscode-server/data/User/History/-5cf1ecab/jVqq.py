...
def gradient_integrated(targets, lenghts, criterion, model_name, model, trial, patient, treshold_labels, ig_gradients):
    ...
    label_names_count = ['General C.', 'Shoulder C.', 'Shoulder Elevation', 'Exaggerated Shoulder Abd.', 'Trunk C.', 'Head C.']
        ...
        if patient in [3, 11, 17] :
            ...
            if trial in trial_1:
                ...
                for j in trial_batch[trial]:
                    ...
                    active_labels = [label_names_count[m] for m in range(0, len(label_names_count)) if targets[j+32*trial, m] > 0]
                    if active_labels:
                        active_labels_str = ", ".join(active_labels)
                    else:
                        active_labels_str = "No compensation"
                    
                    sns.heatmap(map[j][0:lenghts[j]].T.detach().cpu().numpy(), cmap='RdYlBu', ax=axes[label], cbar_kws={'label': 'Normalized Saliency'})
                    ...
                    head_joints = map[j][0:lenghts[j], :].sum(dim=1).detach().cpu().numpy()
                    ...
                    fig.suptitle(f"{model_name}: Saliency Maps of Patient {patient + 1}:  Trial {j+32*trial} \n Active Labels: {active_labels_str}", fontsize=16, weight='bold')
                    fig.tight_layout(rect=[0, 0, 1, 0.96])
                    fig.savefig(f"saliency_maps/integrated_gradients/{model_name}_patient_{patient}_trial_{j+32*trial}_IG.png", dpi=300)
                    plt.close(fig)
                    
                    fig2.suptitle(f"{model_name}: Sum of Saliency for Patient {patient + 1}:  Trial {j+32*trial}", fontsize=16, weight='bold')
                    fig2.tight_layout(rect=[0, 0, 1, 0.96])
                    fig2.savefig(f"saliency_maps/integrated_gradients/{model_name}_patient_{patient}_trial_{j+32*trial}_sum_IG.png", dpi=300)
                    plt.close(fig2)
                
        if label == 0:
            binary_map = (map.sum(dim=2) < treshold_labels).int().detach().cpu().numpy()
