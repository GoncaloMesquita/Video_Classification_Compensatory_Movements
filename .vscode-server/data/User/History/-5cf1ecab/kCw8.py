        ...
        if patient in [3, 11, 17] :
            ...
            if trial in trial_1:
                ...
                for j in trial_batch[trial]:
                    ...
                    fig.suptitle(f"{model_name}: Saliency Maps of Patient {patient + 1}:  Trial {j} \n Active Labels: {active_labels_str}", fontsize=16, weight='bold')
                    fig.tight_layout(rect=[0, 0, 1, 0.96])
                    fig.savefig(f"saliency_maps/integrated_gradients/{model_name}_patient_{patient}_trial_{j}_IG.png", dpi=300)
                    plt.close(fig)
                    
                    fig2.suptitle(f"{model_name}: Sum of Saliency for Patient {patient + 1}:  Trial {j}", fontsize=16, weight='bold')
                    fig2.tight_layout(rect=[0, 0, 1, 0.96])
                    fig2.savefig(f"saliency_maps/integrated_gradients/{model_name}_patient_{patient}_trial_{j}_sum_IG.png", dpi=300)
                    plt.close(fig2)
                
        if label == 0:
            binary_map = (map[:][0:lenghts[0]].sum(dim=2) < treshold_labels).int().detach().cpu().numpy()
