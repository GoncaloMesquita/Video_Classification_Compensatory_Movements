...
def create_model(model_name, input_size, hidden_size, num_layers, num_labels, dropout, checkpoint, mode, pretrained):
    ...
    if model_name == 'LSTM':
        ...
        #          embed_dim=64, num_people=1, num_frames=770, num_points=33, kernel_size=7, num_heads=32,
        #          type_1_size=(1, 1), type_2_size=(1, 1), type_3_size=(1, 1), type_4_size=(1, 1),
        #          attn_drop=0., head_drop=0., drop=dropout, rel=True, drop_path=0., mlp_ratio=4.,
        #          act_layer=nn.GELU, norm_layer_transformer=nn.LayerNorm, index_t=False, global_pool='avg')
            
            model = SkateFormer( in_channels=3, depths=(2, 2, 2, 2), channels=(96, 192, 192, 192), num_classes=6,
                 embed_dim=96, num_people=2, num_frames=64, num_points=33, kernel_size=7, num_heads=32,
                 type_1_size=(8, 12), type_2_size=(8, 12), type_3_size=(8, 12), type_4_size=(8, 12),
                 attn_drop=0., head_drop=0., drop=0., rel=True, drop_path=0., mlp_ratio=4.,
                 act_layer=nn.GELU, norm_layer_transformer=nn.LayerNorm, index_t=False, global_pool='avg')
            
            checkpoints = torch.load("models/SkateFormer_NTU120_j.pt", weights_only=True)
            
            for k in [
                'head.weight', 'head.bias', 'joint_person_embedding',
                'stages.0.blocks.0.transformer.gconv', 'stages.0.blocks.1.transformer.gconv',
                'stages.1.blocks.0.transformer.gconv', 'stages.1.blocks.1.transformer.gconv',
                'stages.2.blocks.0.transformer.gconv', 'stages.2.blocks.1.transformer.gconv',
                'stages.3.blocks.0.transformer.gconv', 'stages.3.blocks.1.transformer.gconv'
            ]:
                if k in checkpoints:
                    print(f"Removing key {k} from pretrained checkpoint")
                    del checkpoints[k]
