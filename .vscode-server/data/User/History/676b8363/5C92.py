...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample_data_tensor = sample_data.clone().detach().float()  # Adjust dtype as needed
        sample_target_tensor = sample_target.clone().detach().float()
...
def collate_fn(batch):
    ...
    # padded_sequences = pack_padded_sequence(padded_sequences, lengths, batch_first=True, enforce_sorted=False)
...
def load_data(data_dir_labels, data_dir_skeletons):
    ...
    ske = torch.load(data_dir_skeletons)
    labels = torch.load(data_dir_labels)
