...
def create_dataloader(x, y, batch_size, shuffle, model_name, trainII, x2):
    ...
    if trainII:
        ...
        dataset = CustomDataset(x, y, model_name, trainII, x2) 
        dataloader = DataLoader(dataset,batch_size=batch_size, collate_fn=collate_fn, shuffle=shuffle, num_workers=4, pin_memory=True)
...
def collate_fn(batch):
    ...
    padded_ske = []
    padded_labels = []
    for seq, lab in sequences, labels:
        # if len(seq) > max_length:
        #     max_length = len(seq)
        seq = torch.tensor(np.array(seq))
        lab = torch.tensor(np.array(lab))
        pad_length = max_length - len(seq)
        if pad_length > 0:
            pad_frames = seq[0].unsqueeze(0).repeat(pad_length, *[1 for _ in seq[0].shape])  
            pad_labels = torch.ones(pad_length)
            # Concatenate seq and the padding frames
            padded_seq = torch.cat((seq, pad_frames), dim=0)
            padd_label = torch.cat((lab, pad_labels), dim=0)
        else:
            padded_seq = seq[0:max_length]
            padd_label = labels[0:max_length]
        padded_ske.append(padded_seq)
        padded_labels.append(padd_label)
    padded_sequences = torch.stack(padded_ske)
    padded_annotations = torch.stack(padded_labels)

    if sequences2[0] is None:
        return padded_sequences, padded_annotations, lengths, None
...
def initial_distance(skeletons):
    ...
    for sequence in skeletons:
        ...
        for frame in sequence:
            ...
            feature_sequence.append(distance.reshape(75))
...
def load_data(data_dir_labels, data_dir_skeletons, num_patients):
...
def load_pseudo_label(data_dir_skeletons, data_true_dir, data_pseudo_dir, first_label, true_labels, model_name, num_patients):
    ...
    pseudo_data_set_info(data_true, data_pseudo, data_pseudo_dir, num_patients)
    cross_val_data = []
...
def pseudo_data_set_info(dt, dp, save_dir, num_patients):

    data_true_list = torch.tensor([frame for i in range(1, num_patients) for trial in dt[i] for frame in trial])
    data_pseudo_list = torch.tensor([frame for i in range(1, num_patients) for trial in dp[i] for frame in trial])
