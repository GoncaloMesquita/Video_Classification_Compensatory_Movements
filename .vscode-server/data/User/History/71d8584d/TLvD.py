...
from models.moment import Moment_II    
...
def create_model(model_name, input_size, hidden_size, num_layers, num_labels, dropout, checkpoint, mode, pretrained, device, learning_rate, eta, epochs, seq_size):
    ...
    if model_name == 'LSTM':
        ...
        if mode == "train":
            ...
            model = SkateFormer( in_channels=3, depths=(2, 2, 2, 2), channels=(96, 192, 192, 192), num_classes=num_labels,
                 embed_dim=96, num_people=1, num_frames=1876, num_points=input_size//3, kernel_size=7, num_heads=32,
        ...
        if mode == 'test':  
            ...
            model = SkateFormer( in_channels=3, depths=(2, 2, 2, 2), channels=(96, 192, 192, 192), num_classes=num_labels,
                 embed_dim=96, num_people=1, num_frames=769, num_points=input_size//3, kernel_size=7, num_heads=32,
                 type_1_size=(1,5), type_2_size=(1,5), type_3_size=(1,5), type_4_size=(1,5),
                 attn_drop=0., head_drop=0., drop=0., rel=True, drop_path=0., mlp_ratio=4.,
                 act_layer=nn.GELU, norm_layer_transformer=nn.LayerNorm, index_t=False, global_pool='avg')
            checkpoints = torch.load(checkpoint, weights_only=True, map_location=device)
            model.load_state_dict(checkpoints, strict=True)
        ...
        if mode == 'train':
            ...
            model = Moment_II(num_labels, dropout, input_size)
            
            for name, param in model.named_parameters():
                # if name.startswith(('moment_model.head.', 'moment_model.encoder.block.7', 'moment_model.encoder.block.6', 'moment_model.encoder.block.5' ,'moment_model.encoder.block.4','moment_model.encoder.block.3','moment_model.encoder.block.2','moment_model.encoder.block.1','moment_model.encoder.block.0' )):
        ...
        if mode == 'train':
            ...
            model.dinov2.encoder.layer = torch.nn.ModuleList(model.dinov2.encoder.layer[:6] + model.dinov2.encoder.layer[12:])
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs, eta_min=eta)
