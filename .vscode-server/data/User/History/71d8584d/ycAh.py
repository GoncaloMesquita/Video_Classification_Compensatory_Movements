...
def create_model(model_name, input_size, hidden_size, num_layers, num_labels, dropout, checkpoint, mode, pretrained, device):
    ...
    if model_name == 'LSTM':
        ...
        if mode == 'test':
            ...
            model = MomentDino(num_labels, dropout)
            model.load_state_dict(torch.load(checkpoint, weights_only=True, map_location=device), strict=True)
            
            
    if model_name == 'MLP':
        if mode == 'train':
            model = MLP(input_size, hidden_size1=256, hidden_size2=126, num_labels, dropout)
            
        if mode == 'test':
            model = MLP(input_size, hidden_size, num_layers, num_labels, dropout)
            print("checkpoint", checkpoint)
            model.load_state_dict(torch.load(checkpoint,weights_only=True, map_location=device), strict=True)
