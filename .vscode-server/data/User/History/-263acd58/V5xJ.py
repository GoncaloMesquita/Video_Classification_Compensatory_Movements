...
#     def __init__(self, data, targets, model_name, data2=None):
...
class CustomDataset(Dataset):
    def __init__(self, data, targets, model_name, data2=None, target_size=256, max_frames=None):
        self.data = data
        self.targets = targets
        self.data2 = data2
        self.max_len = max(len(row) for row in data)
    ...
    def load_and_preprocess_frames(self, folder_path):
        ...
        for img_file in frame_files:
            ...
            if img is not None:
                ...
                img = center_crop_square(img, self.target_size)
        ...
        if frames:
            return torch.stack(frames)  # Stack all frames into a single tensor [num_frames, C, H, W]
        else:
            return None  # Return None if no valid frames were found
...
def create_dataloader(x, y, batch_size, shuffle, model_name, x2 = None):

    dataset = CustomDataset(x, y, model_name, x2) 
    dataloader = DataLoader(dataset,batch_size=batch_size, collate_fn=collate_fn, shuffle=shuffle, num_workers=1, pin_memory=True)
...
def collate_fn(batch):
    ...
    if model_name[0] == 'SkateFormer':
        ...
        max_length = 512
        sequences, labels, seuqnece2 = zip(*[(seq, label, sequence2) for seq, label, sequence2 in zip(sequences, labels, sequences2) if len(seq) <= max_length])
    ...
    return padded_sequences, lengths, torch.tensor(labels), padded_sequences2
...
def load_data_video(data_dir_labels, data_trial, index_keep,  resize_dim=(256, 256)):
    ...
    for env in sorted(os.listdir(data_trial), key=lambda x: int(x[1:])):
        env_path = os.path.join(data_trial, env)
        if not os.path.isdir(env_path):
            continue

        zed_path = os.path.join(env_path, "ZED")
        if not os.path.isdir(zed_path):
            continue

        for participant in sorted(os.listdir(zed_path), key=lambda x: int(x[1:])):
            
            if participant not in patient_trials:
                patient_trials[participant] = []
                
            participant_path = os.path.join(zed_path, participant)
            if not os.path.isdir(participant_path):
                continue
            
            if env == 'E1' or env == 'E2':
                for affected in ['affected', 'unaffected']:
                    
                    affected_path = os.path.join(participant_path, affected)
                    if not os.path.isdir(affected_path):
                        continue
                    
                    for trial in sorted(os.listdir(affected_path), key=lambda x: int(x.split('_')[1])):
                        trial_path = os.path.join(affected_path, trial)
                        patient_trials[participant].append(trial_path)
            else:
                for trial in sorted(os.listdir(participant_path), key=lambda x: int(x.split('_')[1])):
...
def plot_auc_curves(targets, predictions, ax, fold, n_splits, mean_fpr, mode):
