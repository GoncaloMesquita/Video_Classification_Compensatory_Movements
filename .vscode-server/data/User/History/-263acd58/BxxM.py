...
def metrics(targets, predictions, mode, output_dir, model_name):
    ...
    precision_micro = precision_score(targets, predictions, average='macro', zero_division=0)
    recall_micro = recall_score(targets, predictions, average='macro', zero_division=0)
    f1_micro = f1_score(targets, predictions, average='macro', zero_division=0)
    ...
    # if mode == 'test':
    # Print metrics
    print(f" Accuracy: {accuracy:.4f}, Precision micro: {precision_micro:.4f}, Recall micro: {recall_micro:.4f}, F1 micro: {f1_micro:.4f}", flush=True)
    print(f"Precision sample: {precision_sample}, Recall sample: {recall_sample}, F1 sample: {f1_sample}", flush=True)
...
def plot_auc_curves(targets, predictions, ax, fold, n_splits, mean_fpr):
    ...
    for i, label in enumerate(label_names):
        ...
        interp_tpr = np.interp(mean_fpr, viz[label].fpr, viz[label].tpr)
        interp_tpr[0] = 0.0
        tprs.append(interp_tpr)
        aucs.append(viz[label].roc_auc)
        folds.append(fold)
...
def plot_auc_test(data, output, model_name, mean_fpr, ax):
    ...
    for i, label in enumerate(label_names):
        ...
        mean_tpr[-1] = 1.0
        mean_auc = np.mean(valid_auc, axis=0) if valid_auc else np.nan
    ...
    for i, label in enumerate(label_names):
        ...
        for j, tpr in enumerate(valid_tprs[i]):
            ...
            ax3[m, n].plot(mean_fpr, tpr, color=colors[j], alpha=0.4, label=f'Fold {valid_folds[i][j] + 1} ROC (AUC = {valid_aucs[i][j]:.2f})')
    ...
    m,n = 0 ,0 
    for i, label in enumerate(label_names):
        ax1[m,n].plot(mean_fpr, mean_tprs[i], label=label)
        # colors = plt.cm.viridis(np.linspace(0, 4, len(valid_tprs[i])))
        colors = plt.cm.get_cmap('tab20', len(valid_tprs[i]))
        for j, tpr in enumerate(valid_tprs[i]):

            ax1[m, n].plot(mean_fpr, tpr, color=colors[j], alpha=0.4, label=f'Fold {valid_folds[i][j] + 1} ROC (AUC = {valid_aucs[i][j]:.2f})')
        tprs_upper = np.minimum(mean_tprs[i] + std_tpr, 1)
        tprs_lower = np.maximum(mean_tprs[i] - std_tpr, 0)
    ...
    for i, label in enumerate(label_names):
        ...
        mean_tnr = 1 - mean_fpr
        ax2[m, n].plot(mean_fnr, mean_tnr, label=label)
        colors = plt.cm.get_cmap('tab20', len(valid_tprs[i]))
        # colors = plt.cm.viridis(np.linspace(0, 4, len(valid_tprs[i])))
        for j, tpr in enumerate(valid_tprs[i]):
            fnr = 1 - tpr
            ax2[m, n].plot(mean_tnr, fnr, color=colors[j], alpha=0.4, label=f'Fold {valid_folds[i][j] + 1}')
...
def standardize_skeleton(sequence):
    ...
    sequence.reshape(sequence.shape[0], 33, 3)
    mean = np.mean(sequence, axis=(0, 1))  # Compute mean across frames and joints
    std = np.std(sequence, axis=(0, 1))    # Compute std deviation across frames and joints
