...
def create_dataloader(x, y, batch_size, shuffle, model_name, trainII, x2):
    ...
    if trainII:
        ...
        dataloader = DataLoader(dataset,batch_size=batch_size, sampler=train_sampler, shuffle=False, num_workers=2, pin_memory=True)
    else:
        dataset = CustomDataset(x, y, model_name, trainII, x2) 
        train_sampler = DistributedSampler(dataset, num_replicas=3, rank=[0,1,2])
        dataloader = DataLoader(dataset,batch_size=batch_size, collate_fn=collate_fn, sampler = train_sampler, shuffle=False, num_workers=2, pin_memory=True)
