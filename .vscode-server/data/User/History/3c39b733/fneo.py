        ...
        for batch_idx, (inputs, targets, lengths, inputs2) in enumerate(val_loader):
            ...
            if saliency_map and mode == 'testing' :
                if method == 'ig':
                    ...
                    attributions.append(integrated_gradients(inputs, model, 0, baseline=None, steps=10))
                ...
                del inputs, targets, inputs2, lengths
...
def integrated_gradients(inputs, model, target_label_idx, baseline=None, steps=10):
    if baseline is None:
    ...
    for i in range(steps + 1):
        ...
        loss = outputs[0, target_label_idx].sum()

        loss.backward()

        # Accumulate gradients scaled by the step size

        integrated_grads += scaled_input.grad * (inputs - baseline) / steps
