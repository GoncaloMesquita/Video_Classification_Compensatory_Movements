...
def visualization(model, test_loader, device, model_name, patient, method, trials, n_batch, dataset_name, input_size, num_dimensions):
    ...
    if method == 'ig':
        ...
        gradient_integrated(test_loader, model_name, model, patient, device, trials, n_batch, dataset_name, input_size, num_dimensions)
    
    elif method == 'vg':
        os.makedirs("saliency_maps/vanilla_gradients", exist_ok=True)
        vanilla_gradients(test_loader, model_name, model, patient, device, trials, n_batch, dataset_name, input_size, num_dimensions)
...
def gradient_integrated(test_loader, model_name, model, patient, device, trials, n_batch, dataset_name, input_size, n_dim):
    ...
    atual_trial = [torch.abs(torch.tensor(trial) - n_batch * (atual_batch[i])) if trial > n_batch else torch.tensor(trial) for i, trial in enumerate (trials)]    
        ...
        if batch_idx in atual_batch:
            ...
            if atual_trial[trial_idx] in range(len(inputs)):
                ...
                for i in range(len(label_names)):
                    ...
                    saliency = torch.nn.functional.interpolate(slc.unsqueeze(0), size=(slc.shape[1], input_size // n_dim), mode='nearest').squeeze(0)
...
def vanilla_gradients(test_loader, model_name, model, patient, device, trials, n_batch, dataset_name, input_size, n_dim):
