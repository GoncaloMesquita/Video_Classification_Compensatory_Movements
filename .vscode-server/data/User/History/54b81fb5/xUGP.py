...
def gradient_integrated(test_loader, model_name, model, patient, device, trials, n_batch, dataset_name, input_size):
    ...
    atual_trial = [torch.abs(torch.tensor(trial) % n_batch) if trial < n_batch else torch.tensor(trial) for trial in trials]    atual_trial = sorted(atual_trial, key=lambda x: x.item())
