...
def train_I( args):
    ...
    for i , (X_t, y_t, X_test, y_test) in enumerate (cross_val_data):
        ...
        if args.mode == 'test' and args.saliency_map:
            if args.model_name == 'moment':    
                args.checkpoint = f'Results/moment/moment_bs16_lr1.000e-04_hs1_th0.200_0.400_0.150_0.350_0.150_0.350_eta1.000e-05_ptTrue/saved_models/moment_{i}_best.pth'
            elif args.model_name == 'LSTM':
                args.checkpoint = f'Results/LSTM/LSTM_newdata_cross_bs16_lr6.161e-04_hs192_th0.500_0.400_0.500_0.350_0.300_0.100_eta9.548e-05_ptTrue/saved_models/LSTM_{i}_best.pth'
            elif args.model_name == 'AcT':
                args.checkpoint = f'Results/AcT/AcT_bs16_lr2.900e-06_hs1_th0.200_0.400_0.150_0.350_0.150_0.350_eta8.270e-04_ptTrue/saved_models/AcT_{i}_best.pth'
            elif args.model_name == 'SkateFormer':
                args.checkpoint = f'Results/SkateFormer/SkateFormer_bs16_lr3.190e-05_hs1_th0.200_0.400_0.150_0.350_0.150_0.350_eta3.670e-06_ptTrue/saved_models/SkateFormer_{i}_best.pth'  
        ...
        if torch.cuda.device_count() > 1:
            # model = nn.DataParallel(model, device_ids=list(range(torch.cuda.device_count())))
            model = nn.DataParallel(model, device_ids=[2,3,4,5,6,7])
            # if isinstance(model, torch.nn.DataParallel):
            #         model = model.module
        ...
        if args.saliency_map:
            ...
            if args.saliency_map and args.model_name == 'moment':
                path = f'dataset/pseudo_labels/{args.model_name}/{args.method}_{args.treshold_labels}'
                os.makedirs(path, exist_ok=True)
                with open(os.path.join(path, f'pseudo_labels_{m}.pkl'), 'wb') as f:
                    pickle.dump(pseudo_labels[m], f)
        else:   
