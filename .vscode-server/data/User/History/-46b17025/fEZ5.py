...
def train_I(args):
    ...
    for i , (X_t, y_t, X_test, y_test) in enumerate (cross_val_data):
        ...
        if args.mode == 'test' or mode == 'test':    
            ...
            model = create_model(args.model_name, args.input_size, args.hidden_size, args.num_layers, args.num_labels, args.dropout, args.checkpoint, "test", args.pretrained, device,  args.learning_rate, args.eta, args.epochs )
            model.to(device)
            if torch.cuda.device_count() > 1:
                model = nn.DataParallel(model, device_ids=list(range(torch.cuda.device_count())))
...
def train_II(args):
    ...
    for i , (X_t, y_t, X_test, y_test) in enumerate (cross_val_data):
        ...
        early_stopping = EarlyStopping(patience=args.patience, model_name=args.model_name, learning_rate=args.learning_rate,batch_size = args.batch_size, output_dir=fold_save_dir ,verbose=True, delta=args.delta)
        model, criterion,   = create_model(args.model_name, args.input_size, args.hidden_size, args.num_layers, args.num_labels, args.dropout, args.checkpoint, args.mode, args.pretrained, device)
        model.to(device)
        # model = nn.DataParallel(model, device_ids=list(range(torch.cuda.device_count())))
