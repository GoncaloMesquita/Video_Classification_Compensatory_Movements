    ...
    def forward(self, inputs, mask):
        ...
        if self.pos_emb is None:
            ...
            if mask is not None:
                ...
                padding_mask = torch.cat([torch.zeros((batch_size, 1), device=inputs.device, dtype=torch.long), padding_mask], dim=1)

                # Expand padding mask to the embedding dimension
                padding_mask = padding_mask.unsqueeze(-1).expand(-1, -1, self.d_model)
                pe = pe.masked_fill(padding_mask == 1, 0)
...
class MultiHeadAttention(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v, mask):
        ...
        # scaled_qk =(q.unsqueeze(-1) * k.unsqueeze(-2)).sum(dim=-1) / torch.sqrt(torch.tensor(dk, dtype=torch.float32))
