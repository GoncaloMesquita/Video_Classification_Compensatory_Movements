    ...
    def forward(self, inputs, mask=None):
        ...
        if self.pos_emb is None:
            ...
            if mask is not None:
                ...
                padding_mask = mask.squeeze(1).squeeze(1).bool()  # Shape: [batch_size, seq_len]
                padding_mask = torch.cat([torch.ones((batch_size, 1), device=inputs.device, dtype=torch.bool), padding_mask], dim=1)
...
def create_padding_mask(lengths, max_len):
    mask = torch.arange(max_len).to(lengths.device).expand(lengths.size(0), max_len) < lengths.unsqueeze(1)
...
class AcT(nn.Module):
    ...
    def forward(self, inputs, lengths=None):
        ...
        if lengths is not None:
            mask = create_padding_mask(lengths, max_len).to(x.device)
