...
def load_pseudo_label(data_dir_skeletons, data_true_dir, data_pseudo_dir, first_label, true_labels, model_name):
    ...
    if data_pseudo_dir.rsplit('/', 2)[1].rsplit('_',3) == 3:
        nan_indices = {}
        for key in data_pseudo.keys():
            nan_indices[key] = [i for i, frame in enumerate(data_pseudo[key]) if np.isnan(frame).any()]
            data_pseudo[key] = [frame for i, frame in enumerate(data_pseudo[key]) if i not in nan_indices[key]]
            data_true[key] = [frame for i, frame in enumerate(data_true[key]) if i not in nan_indices[key]]
            ske[key] = [frame for i, frame in enumerate(ske[key]) if i not in nan_indices[key]]

        # Save the indices with NaN labels
        nan_indices_filepath = os.path.join(os.path.dirname(data_pseudo_dir), 'nan_indices.json')
        with open(nan_indices_filepath, 'w') as f:
            json.dump(nan_indices, f, indent=4)
