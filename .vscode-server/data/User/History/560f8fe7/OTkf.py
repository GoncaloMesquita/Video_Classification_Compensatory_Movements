...
def load_pseudo_label(data_dir_skeletons, data_true_dir, data_pseudo_dir, first_label, true_labels, model_name):
    ...
    num_files = len([name for name in os.listdir(data_pseudo_dir.rsplit('/', 1)[0]) if os.path.isfile(os.path.join(data_pseudo_dir, name))])
    for i in range(1, num_files + 1):
        ...
        with open(f"{data_pseudo_dir.rsplit('.', 1)[0][:-1]}{i}.pkl", 'rb') as f:
            ...
            if isinstance(data, list):
                # data_list = [item1[:-1] for sublist in data for item in sublist for item1 in item]  
                data_list = [item1 for sublist in data for item in sublist for item1 in item]  
    ...
    if 'SkateFormer' in data_pseudo_dir.split('/'):
        flattened_vector = {}
        
        for i in range (1,19):

            flattened_vector[i] = [item[0] for item in data_pseudo[i]]
        data_pseudo = flattened_vector
        
    if 'moment' in data_pseudo_dir.split('/') and 'ig_3.0' in data_pseudo_dir.split('/'):
        flattened_vector = {}

        for i in range (1,19):

            flattened_vector[i] = [item1 for item in data_pseudo[i] for item1 in item]
        data_pseudo = flattened_vector
