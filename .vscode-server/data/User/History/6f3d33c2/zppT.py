...
def objective(trial):
    ...
    device_ids = [0,1,2,3]
    # Initialize model
    device = torch.device(f'cuda:0,1,2,3' if torch.cuda.is_available() else 'cpu')
    ...
    for i , (X_t, y_t, X_val, y_val, X_test, y_test) in enumerate (cross_val_data):
        ...
        if args.mode == "train":
            ...
            for epoch in range(args.epochs):
                ...
                val_loss, targets , predictions, _ = validate(model, val_loader, criterion, device, "validation", args.save_dir, args.model_name, args.threshold, args.optuna)
        ...
        model = create_model(args.model_name, args.input_size, args.hidden_size, args.num_layers, args.num_labels, args.dropout, args.checkpoint, "test", args.pretrained)
        model = nn.DataParallel(model, device_ids=device_ids)
        model.to('cuda:1')
